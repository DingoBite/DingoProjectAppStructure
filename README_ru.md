...
`ExternalDependenciesRegistererBase.BindToModelAsync()` объявлен как async-hook, и в текущей реализации `G` этот вызов ожидается через `await` до инициализации visual roots.

Практический вывод:

- этот hook подходит для async-bind-а, первичной загрузки данных и wiring-а между dependencies и model-слоем, которые должны завершиться до `AppViewRoot.InitializeAsync()` и `AppPopupViewRoot.InitializeAsync()`;
- работу, которую можно запускать уже после готовности view-слоя, по-прежнему удобно переносить в `PostInitializeAsync()` или планировать явно на стороне проекта.
